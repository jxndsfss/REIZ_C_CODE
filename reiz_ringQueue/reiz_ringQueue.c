...
/*
    ...
    @return void
*/
extern void ringQueue_Construct(pRingQueue_t pRingQ, uint8_t *bufferArray, size_t arraySize) {
...
/*
    ...
    @return -1     - 读取失败
            count  - 读取后队列中当前存储字节数
*/
extern int32_t ringQueue_GetByte(pRingQueue_t pRingQ, uint8_t *pDst) {
    if (!pRingQ->count) {
        return -1;
    }
...
*/
extern int32_t ringQueue_GetMult(pRingQueue_t pRingQ, uint8_t *pDst, uint32_t num) {
    if(num > pRingQ->count){
        return -1;
    }
...
/*
    ...
    @return count  - 环形队列缓存当前存储字节数
*/
extern uint32_t ringQueue_Peek(pRingQueue_t pRingQ, uint8_t *pDst) {
    *pDst = pRingQ->pBuffer[pRingQ->tail];
    return pRingQ->count;
}
...
/*
    ...
    @return -1     - 溢出
            count  - 环形队列缓存当前存储字节数
*/
extern int32_t ringQueue_PutMult(pRingQueue_t pRingQ, uint8_t *pSrc, uint32_t num) {    
    while(num){
        if (pRingQ->count != pRingQ->length) {
