...
#endif /* REIZ_ELEMENT_QUEUE_GLOBALS */
...
/* 宏值：1为打开，0为关闭 */
#define ELE_OVERFLOW_TIMES_COUNT_ENABLE         1                               //溢出覆盖次数统计功能
#define ELE_MAX_COUNT_ONCE_STORED_ENABLE        1                               //统计队列之前存储过的最大元素个数
#define ELE_GET_PUT_RETURN_COUNT_ENABLE         1                               //存取函数是否返回队列当前存储元素个数

#define ELEMENT_TYPE                            uint32_t                        //队列存储元素数据类型定义
...
extern ELEMENT_TYPE eleQueue_GetElement         ( pEleQueue_t pRingQ );                         //从环形队列取出1个元素
extern ELEMENT_TYPE eleQueue_Peek               ( pEleQueue_t pRingQ );                         //查看环形队列尾端1个元素内容，但不取出该元素
...
#endif /* ELE_GET_PUT_RETURN_COUNT_ENABLE */
...
#endif /* ELE_OVERFLOW_TIMES_COUNT_ENABLE */

#if ELE_MAX_COUNT_ONCE_STORED_ENABLE
extern uint32_t     eleQueue_GetMaxCount        ( pEleQueue_t pRingQ );                         //读取环形队列之前存储过的最大元素数量
#endif /* ELE_MAX_COUNT_ONCE_STORED_ENABLE */
